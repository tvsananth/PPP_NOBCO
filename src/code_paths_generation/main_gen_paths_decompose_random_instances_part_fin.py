...
def get_sols_per_demand(save_dir, data_nodes, data_edges_dir, nodes_demand, nodes_exit, tij_edges_undir, A_mat_2n_noedges, near_exits=25):
    ...
    for i_demand in range(nodes_demand.shape[0]):
        fi_l = np.genfromtxt(save_dir + '/feas_sols_sorted' + str(i_demand) + '.txt', dtype=np.int32)
...
def main_compute():
    ...
    if (not(Path(save_dir + '/rand_ini_sols_dir').is_dir())):   
       os.mkdir(save_dir + '/rand_ini_sols_dir')
    if (not(Path(save_dir + '/rand_ini_fr_dir').is_dir())):   
    ...
    time_SA1 = t_fin - t_ini
    time_SA2 = np.genfromtxt(save_dir + '/times_paths_full_run.txt')
    time_SA = time_SA1 + time_SA2
